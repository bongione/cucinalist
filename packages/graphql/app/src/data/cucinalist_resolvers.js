...
export const cucinalistResolvers = {
    Query: {
    ...
    Ingredient: {
        __resolveType: (parent) => typeof parent.serves === "number"
            ? "Recipe"
            : "BoughtIngredient",
    },
    StepInputIngredient: {
    ...
    StepInputIngredientType: {
        __resolveType: (parent) => parent.amount &&
            typeof parent.amount === "object"
            ? "StepOutputIngredient"
            : "RecipeIngredient",
    },
    StepPrecondition: {
        requiredIngredients: async (parent, args, context) => {
            const linkRecords = await context.prisma.stepPreconditionInputIngredient.findMany({
                where: { stepPreconditionId: parent.id },
            });
            const records = await Promise.all(linkRecords.map((link) => context.prisma.stepInputIngredient.findUnique({
    ...
    CookingStep: {
        process: async (parent, args, context) => {
            const technique = await context.resolverContext.resolveSymbol(parent.techniqueId);
        ...
        requires: async (parent, args, context) => context.prisma.stepInputIngredient.findMany({
            where: { cookingStepId: parent.id },
        }),
        produces: async (parent, args, context) => context.prisma.stepOutputIngredient.findMany({
            where: { cookingStepId: parent.id },
        }),
    },
    RecipeIngredient: {
        ingredient: async (parent, args, context) => {
            const resolved = await context.resolverContext.resolveSymbol(parent.storeBoughtIngredientId || parent.producedByRecipeId);
            if (resolved.type === "StoreBoughtIngredient" ||
                resolved.type === "Recipe") {
                return resolved;
            }
            else if (resolved.type === "UnresolvedId") {
                throw new Error(`Could not resolve ingredient ${String(parent.storeBoughtIngredientId || parent.producedByRecipeId)}`);
            }
            else {
        ...
        unit: async (parent, args, context) => {
            const uom = await context.resolverContext.resolveSymbol(parent.unitOfMeasureId);
            if (uom.type === "UnresolvedId") {
                throw new Error(`Could not resolve unit of measure ${parent.unitOfMeasureId}`);
            }
