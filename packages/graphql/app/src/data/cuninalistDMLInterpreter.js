import { parseCucinalistDsl, } from "@cucinalist/dsl";
...
export async function executeDML(prismaClient, dmlStr) {
    const statements = parseCucinalistDsl(dmlStr);
    const newOrUpdatedEntities = [];
    await prismaClient.$transaction(async (prisma) => {
        ...
        for (const statement of statements) {
            if (statement.type === "CreateContext") {
                ...
                const recipe = await processRecipeStatement(statement, executionContext);
                newOrUpdatedEntities.push(recipe);
            }
            else if (statement.type === 'UnitOfMeasure') {
                const uom = await processCreateUnitOfMeasureStatement(statement, executionContext);
 ...
 */
async function processCreateContextStatement(statement, executionContext) {
    const parentContext = await executionContext.resolveSymbol(statement.parentContext);
    if (parentContext.type === "UnresolvedId") {
        throw new Error(`Parent context ${statement.parentContext} not found`);
    }
    const newContext = await executionContext.createCucinalistContext(statement.id, statement.parentContext);
    if (statement.switchToContext) {
        await executionContext.switchToContext(statement.id);
    }
    return newContext;
}
async function processSwitchToContextStatement(statement, executionContext) {
