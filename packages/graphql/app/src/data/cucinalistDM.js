import { MeasuringFeature, } from "../__generated__/prismaClient";
function recipeContext(executionContext) {
    let ctx = executionContext;
    // @ts-expect-error Violating the readonly property on purpose
    ctx["localResolutions"] = new Map();
    return ctx;
}
export async function processRecipeStatement(recipe, executionContext) {
    ...
    await executionContext.assignSymbol(recipe.id, completedRecord.type, completedRecord.id);
...
async function findOrCreateStoreBoughtIngredient(executionContext, ingredientId) {
    let recipeOrStoreBoughtIngredient = await executionContext.resolveSymbol(ingredientId);
    if (recipeOrStoreBoughtIngredient.type === "UnresolvedId") {
        const newIngredient = await executionContext.prisma.storeBoughtIngredient.create({
            data: {
                gblId: ingredientId,
                name: ingredientId,
            },
        ...
        await executionContext.assignSymbol(ingredientId, recipeOrStoreBoughtIngredient.type, recipeOrStoreBoughtIngredient.id);
    }
    else if (recipeOrStoreBoughtIngredient.type !== "StoreBoughtIngredient" &&
        recipeOrStoreBoughtIngredient.type !== "Recipe") {
        throw new TypeError(`The id ${ingredientId} is not a store bought ingredient or recipe`);
    }
    return recipeOrStoreBoughtIngredient;
}
async function findOrCreateUnitOfMeasure(executionContext, unitId) {
    ...
    if (unitOfMeasure.type === "UnresolvedId") {
        ...
        await executionContext.assignSymbol(unitId, unitOfMeasure.type, unitOfMeasure.id);
    }
...
async function findOrCreateCookingTechnique(executionContext, techniqueId) {
    ...
    if (technique.type === "UnresolvedId") {
        ...
        await executionContext.assignSymbol(techniqueId, technique.type, technique.id);
    }
...
async function processRecipeIngredients(executionContext, recipe, recipeRecord) {
    const recipeIngredientsRecords = [];
    for (let i = 0; i < recipe.ingredients.length; i++) {
        const ri = recipe.ingredients[i];
        const recipeOrStoreBoughtIngredient = await findOrCreateStoreBoughtIngredient(executionContext, ri.ingredientId);
        let unitOfMeasure = await findOrCreateUnitOfMeasure(executionContext, ri.amount.unit);
        const riRecord = await executionContext.prisma.recipeIngredient.create({
            data: {
                recipeId: recipeRecord.id,
                storeBoughtIngredientId: recipeOrStoreBoughtIngredient.type === "StoreBoughtIngredient"
                    ? recipeOrStoreBoughtIngredient.id
                    : undefined,
                producedByRecipeId: recipeOrStoreBoughtIngredient.type === "Recipe"
                    ? recipeOrStoreBoughtIngredient.id
                    : undefined,
...
async function processRecipeSteps(executionContext, recipe, recipeRecord) {
    const recipeStepsRecords = [];
    for (let i = 0; i < recipe.cookingSteps.length; i++) {
        const step = recipe.cookingSteps[i];
        const techniqueRecord = await findOrCreateCookingTechnique(executionContext, step.processId);
        ...
        for (let j = 0; i < step.ingredients.length; i++) {
            ...
            const outputOrIng = executionContext.localResolutions.get(inputIngredient);
            if (!outputOrIng) {
        ...
        for (let j = 0; i < step.produces.length; i++) {
            ...
            const outputRecord = await executionContext.prisma.stepOutputIngredient.create({
                data: {
                    cookingStepId: stepRecord.id,
                    nthOutput: j,
        ...
        for (let j = 0; j < step.preconditions.length; j++) {
            ...
            for (let k = 0; k < precondition.ingredientsNeeded.length; k++) {
                ...
                const ingredientRecord = executionContext.localResolutions.get(ingredientNeeded);
                if (!ingredientRecord) {
...
export async function processCreateUnitOfMeasureStatement(statement, executionContext) {
    ...
    if (existingUnit !== null && existingUnit.type !== "UnitOfMeasure") {
        throw new Error(`Symbol ${statement.id} already exists and is not a UnitOfMeasure`);
    }
    const p = existingUnit === null
        ? executionContext.prisma.unitOfMeasure.create({
            data: {
                gblId: statement.id,
                name: statement.name,
                plural: statement.symbolPlural ? statement.symbolPlural : undefined,
                measuring: Object.values(MeasuringFeature).findIndex((f) => statement.measuring === f) === -1
    ...
    await executionContext.assignSymbol(statement.id, unitRecord.type, unitRecord.id);
    return unitRecord;
}
